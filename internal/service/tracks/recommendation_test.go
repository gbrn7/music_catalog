	...
	tests := []struct {
	...
	}{
		{
			...
			mockFn: func(args args) {
				...
				MocktrackActivitiesRepo.EXPECT().GetBulkBySpotifyIDs(gomock.Any(), uint(1), []string{"3z8h0TU7ReDPLIbEnYhWZb", "4u7EnebtmKWzUH433cf5Qv"}).
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if (err != nil) != tt.wantErr {
				t.Errorf("service.GetRecomendation() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("service.GetRecomendation() = %v, want %v", got, tt.want)
			}
		})
	}
}
