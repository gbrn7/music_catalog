...
func Test_repository_GetUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: false,
			mockFn: func(args args) {
				mock.ExpectQuery(`SELECT \* FROM "users" .+`).WithArgs(args.email, args.username, args.id, 1).WillReturnRows(sqlmock.NewRows([]string{"id", "created_at", "updated_at", "email", "username", "password", "created_by", "updated_by"}).AddRow(1, now, now, "test@gmail.com", "testusername", "password", "test@gmail.com", "test@gmail.com"))
			},
		},
		{
			name: "failed",
